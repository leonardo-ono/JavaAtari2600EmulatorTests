...
public class Cpu {
    ...
    public static void setFlag(int bit, int value) {
        ...
        //if (bit == FLAG_BIT_OVERFLOW && value == 1) {
        //    System.out.println("delete later setFlag() crty=" + Tia.crtY);
        //}
    }
    ...
    public static void ADC(int op) {
        int f = get(op);
        int carryIn = getFlag(FLAG_BIT_CARRY);

        int tmp = RA + f + carryIn;

        // Cálculo do overflow (sempre binário)
        setFlag(FLAG_BIT_OVERFLOW, ((~(RA ^ f) & (RA ^ tmp)) & 0x80) != 0 ? 1 : 0);
        ...
        // ---- MODO DECIMAL (BCD) ----
        int al = (RA & 0x0F) + (f & 0x0F) + carryIn;
        int ah = (RA & 0xF0) + (f & 0xF0);
        ...
        // Resultado final da operação BCD
        tmp = (al & 0x0F) | (ah & 0xF0);

        // Carry em BCD (não é o bit 8 direto!)
        setFlag(FLAG_BIT_CARRY, (ah & 0x100) != 0 ? 1 : 0);
...
//        139 high 337
    ...
    public static void SBC(int op) {
        int value = get(op);
        int carry = getFlag(FLAG_BIT_CARRY);     // 1 = sem empréstimo; 0 = com empréstimo
        int a = RA;

        if ((RP & 0b1000) != 0) {
            // --------- MODO DECIMAL (BCD) ----------
            int al = (a & 0x0F) - (value & 0x0F) - (carry == 0 ? 1 : 0);
            int ah = (a >> 4) - (value >> 4);
            ...
            if (ah < 0) {
                ah += 10;
                setFlag(FLAG_BIT_CARRY, 0); // houve empréstimo
            } else {
                setFlag(FLAG_BIT_CARRY, 1);
            }

            int result = ((ah % 10) << 4) | (al % 10);
            RA = result & 0xFF;

            // flags Z e N são baseadas no valor binário (igual o 6502 real)
            setFlag(FLAG_BIT_ZERO, (RA == 0) ? 1 : 0);
            setFlag(FLAG_BIT_NEGATIVE, (RA & 0x80) != 0 ? 1 : 0);

            // Overflow em modo decimal é calculado igual ao binário
            setFlag(FLAG_BIT_OVERFLOW, (((a ^ result) & (a ^ value)) & 0x80) != 0 ? 1 : 0);

        } else {
            // --------- MODO BINÁRIO (normal) ----------
            int m = value ^ 0xFF;  // complemento
            int tmp = a + m + carry;

            setFlag(FLAG_BIT_CARRY, (tmp & 0x100) != 0 ? 1 : 0);
            setFlag(FLAG_BIT_ZERO, (tmp & 0xFF) == 0 ? 1 : 0);
            setFlag(FLAG_BIT_OVERFLOW, ((a ^ tmp) & (m ^ tmp) & 0x80) != 0 ? 1 : 0);
            setFlag(FLAG_BIT_NEGATIVE, (tmp & 0x80) != 0 ? 1 : 0);
